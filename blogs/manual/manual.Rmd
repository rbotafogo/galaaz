...
# Bridge default 60s; Arrow/dplyr pipes may need more.
ENV['GALAAZ_BRIDGE_TIMEOUT_SEC'] ||= '300'
# Writable jobs dir for R::Job demos when knitting (avoid Permission denied).
ENV['GALAAZ_JOBS_DIR'] ||= File.join(Dir.tmpdir, 'galaaz_jobs_manual')
...
bin_tbl <- function(df) {
  k <- knitr::kable(
    df, row.names = FALSE, booktabs = TRUE, linesep = "",
    col.names = c("Script", "Role", "2.0?"))
  if (knitr::is_latex_output()) {
    k <- kableExtra::kable_styling(
      k, font_size = 9, latex_options = "scale_down")
...
df_boot <- data.frame(
  Script = c(
    "galaaz-bootstrap", "galaaz-jruby",
    "galaaz_jruby_env.inc.sh", "install-tinytex"),
  Role = c(
    "WSL2 helper: Docker checks; optional TinyTeX/poppler.",
    "JRuby with repo lib/ on LOAD_PATH and JVM flags.",
    "Sourced by bash wrappers; sets JRUBY_J_ARGS.",
...
df_gk <- data.frame(
  ...
  Role = c(
    ...
    "Drafts from rticles templates; legacy polyglot wrapper.",
...
df_ts <- data.frame(
  Script = c(
    "run_rspec", "run_all_rspec", "run_slow_rspec",
    "run_old_rspec", "run_rspec_subset"),
  Role = c(
    "Top-level specs/*_spec.rb with spec_helper (see docs/testing.md).",
    "Compile ext/new_bridge; run specs/ and new_bridge_specs/.",
...
df_ot <- data.frame(
  ...
  Role = c(
    "Graal-era launcher: polyglot ruby --jvm; prefer galaaz-jruby.",
