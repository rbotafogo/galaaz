...
```{ruby class_cleanup, echo=FALSE}
# Drop classes left from a prior output format in one gknit process.
# Chunks run via KnitrEngine#instance_eval, so classes live under RC.
mods = [Object]
mods << RC if defined?(RC)
%w[
  Trajectories TrajectoriesBis TrajPartitioned
  Partition PartitionFather PartitionSimple PartitionEval
].each do |name|
  mods.each do |mod|
