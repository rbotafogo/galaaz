...
echo "[cold-install-cruby] ruby=$(ruby -v)"
echo "[cold-install-cruby] R=$(R --version | head -n 1)"
gem sources --add https://rubygems.org/ >/dev/null 2>&1 || true

SOURCE="${GALAAZ_COLD_INSTALL_SOURCE:-local}"
if [[ "${SOURCE}" == "rubygems" ]]; then
  echo "[cold-install-cruby] gem install galaaz from rubygems.org"
  gem install --source https://rubygems.org galaaz
else
  gem_file="$(ls -1 /dist/galaaz-*.gem 2>/dev/null | head -n 1 || true)"
  if [[ -z "${gem_file}" ]]; then
    echo "cold-install-cruby: mount the built gem at /dist/galaaz-*.gem" >&2
    exit 1
  fi
  echo "[cold-install-cruby] gem install ${gem_file} (msgpack from rubygems.org)"
...
echo "[cold-install-cruby] gem install rspec (dev dependency, not in the runtime gem)"
gem install --source https://rubygems.org rspec -v '~> 3.8'

echo "[cold-install-cruby] R packages used by fast specs (dplyr, knitr, rmarkdown, arrow)"
...
echo "[cold-install-cruby] fast specs from installed gem (no repo mount, no bundle)"
